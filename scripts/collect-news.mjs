...
const sources = [{ name: "中国政府网", url: "https://www.gov.cn/zhengce/index.htm", weight: 3 }, { name: "人力资源和社会保障部", url: "https://www.mohrss.gov.cn/SYrlzyhshbzb/zwgk/zcwj/", weight: 4 }, { name: "工业和信息化部", url: "https://www.miit.gov.cn/zwgk/zcwj/wjfb/index.html", weight: 3 }, { name: "国家发展改革委", url: "https://www.ndrc.gov.cn/xwdt/tzgg/", weight: 2 }];
...
function parse(html, source, date) { const results = []; const expression = /<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi; for (const match of html.matchAll(expression)) { const title = text(match[2]); const tags = tagsFor(title); const index = match.index ?? 0; const context = html.slice(Math.max(0, index - 240), index + match[0].length + 240); const isGraduatePolicy = tags.includes("政策"); if (!context.includes(date) || title.length < 12 || title.length > 70 || title.endsWith("。") || tags.length === 0) continue; const url = new URL(match[1], source.url).toString(); const summary = isGraduatePolicy ? `与毕业求职或独立生活直接相关的${source.name}公开政策。` : `来自${source.name}的公开信息，已按主题相关性纳入昨日热点候选。`; if (!results.some((item) => item.url === url || item.title === title)) results.push({ title, source: source.name, publishedAt: date, summary, tags, url, score: source.weight + tags.length + (isGraduatePolicy ? 8 : 0) }); } return results; }
...
await writeFile("data/daily-news.json", `${JSON.stringify({ targetDate: collected.length ? date : previous.targetDate, status: collected.length ? "已按北京时间汇总昨日资讯；政策优先毕业求职与生活相关内容" : "昨日采集暂不可用，正在展示最近一次成功榜单", items }, null, 2)}\n`);
