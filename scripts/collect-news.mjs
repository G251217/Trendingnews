...
function parse(html, source, date) { const results = []; const expression = /<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi; for (const match of html.matchAll(expression)) { const titleAttribute = match[0].match(/\btitle=["']([^"']+)["']/i); const title = text(titleAttribute?.[1] ?? match[2]); const tags = tagsFor(title); const index = match.index ?? 0; const context = html.slice(Math.max(0, index - 240), index + match[0].length + 240); const isGraduatePolicy = tags.includes("政策"); const isJinan = source.name.startsWith("济南"); if (!context.includes(date) || title.length < 12 || title.length > 70 || title.endsWith("。") || tags.length === 0) continue; const url = new URL(match[1], source.url).toString(); const summary = isGraduatePolicy ? `${isJinan ? "面向济南" : "与"}毕业求职或独立生活直接相关的${source.name}公开政策。` : `来自${source.name}的公开信息，已按主题相关性纳入昨日热点候选。`; if (!results.some((item) => item.url === url || item.title === title)) results.push({ title, source: source.name, publishedAt: date, summary, tags, url, score: source.weight + tags.length + (isGraduatePolicy ? 8 : 0) + (isJinan ? 4 : 0) }); } return results; }
