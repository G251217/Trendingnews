...
const existing = async () => { try { return JSON.parse(await readFile("data/daily-news.json", "utf8")); } catch { return { targetDate: null, items: [] }; } };
...
const status = collected.length ? "已按北京时间汇总今日资讯；就业优先济南与杭州机会" : previousItems.length ? "今日采集暂不可用，正在展示最近一次成功榜单" : "今日暂未找到符合条件的公开资讯";
await writeFile("data/daily-news.json", `${JSON.stringify({ targetDate: collected.length ? date : (previous.targetDate ?? date), status, items }, null, 2)}\n`);
